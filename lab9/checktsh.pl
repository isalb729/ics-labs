...
sub check_trace11 {
	...
	while ( $tshlineold eq "" ) {
       	   $tshlineold = <TSHFILE>;
           if (not defined($tshlineold)) { last; }
     	   $tshlineold =~s/\x0A//g;
	   $tshlineold =~s/\x0D//g;
        }
...
sub check_trace {
    ...
    alarm $timeout;
    my $TSHPID = open(TSH, "$driver -t $tracefile -s $tsh -a '-p'|")
	or die "$0: ERROR: Couldn't run driver on $tsh\n";
    local $SIG{ALRM} = sub { kill(9, $TSHPID); die "Timeout\n" };
    while ($line = <TSH>) {
	if ($verbose) {
	    print $line;
	}
	...
	while ($tshreflineold = <TSHREFFILE>) {
        ...
        # check redundant output of student program
        $tshlineold = "";
	while( $tshlineold eq "") {
            $tshlineold = <TSHFILE>;
            if (not defined($tshlineold)) { $tshline = ""; last; }
     	    $tshlineold =~s/\x0A//g;
	    $tshlineold =~s/\x0D//g;
        }
	if ("" ne $tshline) {
